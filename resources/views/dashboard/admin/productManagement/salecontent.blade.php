                                  ...
                                  <div class="input-group">
                                    <select name="customer_id" class="form-control form-control-sm select2bs4" id="customer_selected">
                                      @foreach ($customers as $customer)
                                        <option value="{{ $customer->parent_id }}" {{ ($customer->is_walkin == 1) ? 'selected' : '' }}>{{ $customer->customer_name }}</option>
                                      @endforeach
                                    </select>
                                    <div class="input-group-append">
                                      <span class="input-group-text text-success" data-toggle="modal" data-target="#addcustomers"><i class="fas fa-plus"></i></span>
                                    </div>
                                  </div>  
                                </div>
                              </div>
                            ...
                            <table id="cart-table" class="table table-bordered">
                              ...
                              <tfoot>
                                ...
                                <tr id="cheque-row" style="display: none;">
                                  ...
                                  <td colspan="2">
                                    <input type="text" class="form-control" id="chequenumber" placeholder="Cheque No" style="max-width: 100px;" value="">
                              ...
                              <form action="{{ route('admin.save') }}" method="post" id="sale-product-form"  autocomplete="off">
                                @csrf
                      ...
                      <div class="card card-success " id="cards">
                        ...
                        <div class="card-body">
                          <div class="row">
                            @forelse ($pdts as $pdt)
                            <div class="col-md-2">
                              <div class="pdt-single " data-id="{{ $pdt->id }}"  data-name="{{ $pdt->product_title }}" data-price="{{ $pdt->sell_price }}">
                                <img class="img-thumbnail" style="height:100px;width:100px" src="{{ $pdt->stckpdt_image ? 'storage/images/products/'.$pdt->stckpdt_image : 'https://placehold.jp/100x100.png' }}" alt="">
                                <h5>{{ $pdt->product_title }}</h5>
                                <p>Tk. {{ $pdt->sell_price }} <br><small style="font-weight:bold"> Qty. {{ $pdt->quantity .' '. $pdt->unit_name}}</small></p>
                              </div>
                            </div>
                            @empty
                              <div class="col-md-12">
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    $.each(response, function(index, value) {
                        ...
                        results += '<li data-id="'+value.id+'"  data-name="'+value.product_title+'" data-price="'+value.sell_price+'">'+value.product_title+' - ৳'+value.sell_price+'</li>';
                    });
