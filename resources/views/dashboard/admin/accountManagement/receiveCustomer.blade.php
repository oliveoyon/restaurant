...
@section('title', 'Received from Customer')
@push('admincss')
...
@section('content')
      ...
      <div class="container-fluid">
        ...
        <div class="card card-success " id="cards">
          <div class="card-header">
            <h3 class="card-title">Receive From Customers</h3>

          </div>
          <!-- /.card-header -->
          <div class="card-body">
            <form action="{{ route('admin.getCusPayment') }}" method="post" id="getCusPayment" class="agetCusPayment" autocomplete="off">
              @csrf
            ...
            <div class="row">
                <div class="col-md-4">
                    ...
                    <div class="form-group row">
                      <label for="colFormLabelSm" class="col-sm-3 col-form-label col-form-label-sm">Payment Method</label>
                      <div class="col-sm-9">
  ...
  <!-- /.content-wrapper -->
         ...
         <div class="modal fade editBrand modal3" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true" data-keyboard="false" data-backdrop="static">
            <div class="modal-dialog modal-dialog-centered" role="document">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                         <form action="{{ route('admin.updateCustomerPayment'); }}" method="post" id="update-cus-payment-form">
                            @csrf
                            ...
                            <div class="form-group">
                                ...
                                <select name="accounts" class="form-control" id="">
                                    ...
                                    @foreach ($accounts as $acc)
                                      @if ($acc->account_name == 'Bank Cheque')
                                        @php
                                          continue
                                        @endphp
                                      @endif
        ...
        <div class="modal fade editBrand1 modal3" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true" data-keyboard="false" data-backdrop="static">
          <div class="modal-dialog modal-dialog-centered" role="document">
              <div class="modal-content">
                  <div class="modal-header">
                      <h5 class="modal-title" id="exampleModalLabel">Payment to Customer</h5>
                  ...
                  {{-- {{ route('admin.updatecategoryDetails'); }} --}}
                  <div class="modal-body">
                       <form action="{{ route('admin.updateCustomerPaymentChq'); }}" method="post" id="update-cus-payment-form1">
                          @csrf
                           <input type="hidden" name="sid">
                          <div class="form-group">
                              <label for="">Total Due</label>
        ...
        <div class="modal fade modal-fullscreen modal2" id="exampleModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
      ...
      $(function(){
        ...
        $('#getCusPayment').on('submit', function(e){
            ...
            $(document).on('click','#editBrandBtn', function(){
                ...
                $.post("{{ route('admin.getCustomerPaymentDetails') }}",{invoice_no:invoice_no}, function(data){
                    ...
                    $('.editBrand').find('input[name="total"]').val(data.details[0].total);
            ...
            $(document).on('click','#editBrandBtn1', function(){
                var invoice_no = $(this).data('id');
                $('.editBrand1').find('form')[0].reset();
                $('.editBrand1').find('span.error-text').text('');
                $.post("{{ route('admin.getCustomerPaymentDetailsChq') }}",{invoice_no:invoice_no}, function(data){
                    $('.editBrand1').find('input[name="sid"]').val(invoice_no);
                    $('.editBrand1').find('input[name="totaldue"]').val(data.details[0].due);
                    $('.editBrand1').find('input[name="payment"]').val(data.details[0].due);
                    $('.editBrand1').find('input[name="total"]').val(data.details[0].due);
                    $('.editBrand1').find('input[name="discount"]').val(data.details[0].discount);
                    $('.editBrand1').find('input[name="paid"]').val(data.details[0].paid);
                   
                    $('.editBrand1').modal('show');
                },'json');
            });
            ...
            $('#update-cus-payment-form1').on('submit', function(e){
                e.preventDefault();
                var form = this;
                $.ajax({
                    url:$(form).attr('action'),
