...
<html>
    ...
    <body>
        <div class="head">
            ...
            <p style="font-size:18px; font-weight: bold; mts-5;">Sales Return</p>
        ...
        <table class="items" width="100%" style="font-size: 14px; border-collapse: collapse;" cellpadding="8">
            ...
            <tbody>
                ...
                @php
                    ...
                    $date1 = new DateTimeImmutable($pdt->created_at);
                    $rdate = $date1->format('F j, Y');
                    $sum += $pdt->rate *  $pdt->quantity;
                @endphp
                <tr>
                    <td>{{ $loop->iteration }}</td>
                    <td>{{ $pdt->product_title }}</td>
                    <td>{{ $pdt->customer_name }}</td>
                    <td>{{ $rdate }}</td>
                    <td>{{ $pdt->barcode }}</td>
                    <td>{{ $pdt->serial_no }}</td>
                    <td>{{ $pdt->batch_no }}</td>
                    ...
                    <td style="text-align: right">{{ $pdt->rate *  $pdt->quantity}}</td>
